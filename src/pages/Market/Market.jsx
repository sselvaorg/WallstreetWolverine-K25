...
function Market() {
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white flex flex-col items-center p-2">
      ...
      <div className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-gray-800 p-4 rounded-lg shadow-lg h-fit">
          ...
          <div className="bg-gray-700 flex flex-col h-fit p-3 text-sm text-justify items-center justify-center rounded-lg">
            {stocks
              .slice((newsPage - 1) * newsPerPage, newsPage * newsPerPage)
              .map((stock) => (
                <div
                  key={stock.name}
                  className="mb-4 p-2 bg-gray-800 rounded-lg w-full text-center"
                >
                  <h3 className="text-sm font-bold text-white">{stock.name}</h3>
                  {currentTime >= new Date(stock.headline1) && (
                    <p className="text-blue-500 text-justify text-sm">
                      {stock.news1}
                    </p>
                  )}
          ...
          {stocks
            ...
            .map((company) => (
              <button
                ...
                className="w-full mb-2 p-3 text-left my-2 border border-gray-600 rounded-lg bg-gray-700 hover:bg-gray-600 transition"
              >
                {company.name}
              </button>
            ))}
